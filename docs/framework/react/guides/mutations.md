...
TanStack Query의 `useMutation` 훅은 데이터 생성, 업데이트, 삭제와 같은 서버 사이드 이펙트를 수행하는 데 사용됩니다. 주요 기능은 다음과 같습니다:

- **상태 관리**: `isIdle`, `isPending`, `isError`, `isSuccess` 상태를 통해 뮤테이션의 진행 상황을 추적할 수 있습니다.
- **변수 전달**: `mutate` 함수를 통해 뮤테이션 함수에 변수를 전달할 수 있습니다.
- **상태 초기화**: `reset` 함수를 사용하여 뮤테이션의 `error` 또는 `data` 상태를 초기화할 수 있습니다.
- **사이드 이펙트**: `onMutate`, `onError`, `onSuccess`, `onSettled` 옵션을 통해 뮤테이션의 각 단계에서 사이드 이펙트를 처리할 수 있습니다.
- **연속 뮤테이션**: `mutate` 함수에 전달된 콜백은 컴포넌트가 마운트된 경우에만 실행됩니다.
- **비동기 처리**: `mutateAsync`를 사용하여 성공 시 해결되거나 오류 시 예외를 발생시키는 프로미스를 반환할 수 있습니다.
- **재시도**: 기본적으로 뮤테이션은 오류 시 재시도하지 않지만, `retry` 옵션을 통해 재시도를 설정할 수 있습니다.
- **오프라인 지원**: 뮤테이션은 저장소에 지속될 수 있으며, 나중에 다시 시작할 수 있습니다.
- **뮤테이션 스코프**: 동일한 `scope.id`를 가진 뮤테이션은 직렬로 실행되어 큐에 추가됩니다.

이러한 기능들은 뮤테이션을 강력하고 유연하게 만들어, 다양한 서버 사이드 이펙트를 효율적으로 처리할 수 있도록 돕습니다.
