...
TanStack Query의 `invalidateQueries` 메소드는 쿼리를 지능적으로 무효화하고 필요에 따라 다시 가져올 수 있도록 합니다. 주요 기능은 다음과 같습니다:

- **쿼리 무효화**: `invalidateQueries`를 사용하여 특정 쿼리 키를 가진 쿼리를 무효화할 수 있습니다. 이는 쿼리가 오래되었거나 사용자의 행동으로 인해 데이터가 변경된 경우에 유용합니다.
- **부분 쿼리 매칭**: 쿼리 키의 접두사나 특정 변수를 사용하여 여러 쿼리를 무효화할 수 있습니다.
- **정확한 매칭**: `exact: true` 옵션을 사용하여 정확히 일치하는 쿼리만 무효화할 수 있습니다.
- **조건부 무효화**: `predicate` 함수를 사용하여 쿼리 캐시의 각 쿼리에 대해 무효화 여부를 결정할 수 있습니다.

이러한 기능들은 쿼리의 데이터 일관성을 유지하고, 사용자가 최신 데이터를 볼 수 있도록 보장합니다.
